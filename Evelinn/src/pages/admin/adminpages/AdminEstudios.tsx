...
const EMPTY = {
  ...
  fecha: "",
  estado: "Completado",
  etiquetas: "", // Lo pedimos como string separado por comas, lo convertiremos a array al guardar
  orden: 0,
};

export const AdminEstudios = () => {
  // Asegúrate de tener estas funciones y estado en tu Contexto/Zustand
  const { estudios = [], addEstudio, deleteEstudio, loading } = useStore();
  ...
  if (loading) {
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center gap-3 text-slate-400">
        ...
        <p className="text-sm font-medium">Cargando estudios desde Supabase...</p>
      </div>
    );
  }
  ...
  const onSubmit = async () => {
    if (!form.titulo.trim()) {
      setErrMsg("El título del estudio es obligatorio");
      return;
    }
    ...
    try {
      ...
      const estudioData = {
        ...
        fecha: form.fecha,
        estado: form.estado,
        // Convertimos el string separado por comas en un array de textos
      ...
      setErrMsg(err?.message || "Error al guardar el estudio");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6 text-slate-100">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-0.5 flex items-center gap-2">
              ...
              {(estudios || []).length} registro{(estudios || []).length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-xl overflow-hidden">
          {(estudios || []).length === 0 ? (
            <div className="p-12 text-center flex flex-col items-center justify-center gap-3">
              ...
              >
                <Plus className="w-4 h-4" /> Añadir mi primer estudio
              </button>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-800 bg-slate-950/40 text-slate-400 text-xs uppercase tracking-wider">
                    <th className="py-4 px-6 font-semibold">Estudio</th>
                    <th className="py-4 px-6 font-semibold">Estado / Fecha</th>
                    <th className="py-4 px-6 font-semibold">Etiquetas</th>
                    <th className="py-4 px-6 font-semibold">Orden</th>
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <div className="flex flex-col gap-1">
                          <span className="inline-flex w-fit items-center px-2 py-0.5 rounded text-[10px] font-medium bg-slate-800 text-slate-300">
                            {e.estado || "N/A"}
                          </span>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex flex-wrap gap-1">
                          {(e.etiquetas || []).map((tag: string, i: number) => (
                            <span
                              key={i}
                              className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/20"
                            >
                              {tag}
                            </span>
                          ))}
                      ...
                      <td className="py-4 px-6 text-right">
                        <button
                          onClick={() => e.id && deleteEstudio(e.id)}
      ...
      {/* VISTA 2: FORMULARIO DE NUEVO ESTUDIO */}
      {vista === "nuevo" && (
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl max-w-4xl mx-auto">
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-1.5 md:col-span-2">
                <label className="text-xs font-semibold text-slate-300 flex items-center gap-2">
                  <GraduationCap className="w-3.5 h-3.5 text-indigo-400" /> Título de la formación *
                </label>
                <input
                  name="titulo"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300 flex items-center gap-2">
                  <Calendar className="w-3.5 h-3.5 text-indigo-400" /> Rango de Fechas
                </label>
                <input
                  name="fecha"
                  placeholder="Ej: Sept 2018 - Jun 2022"
                  value={form.fecha}
                  onChange={onChange}
                  className="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
                />
              ...
              <div className="space-y-1.5">
                ...
                <input
                  name="estado"
                  placeholder="Ej: Completado, En curso, Pausado..."
                  value={form.estado}
                  onChange={onChange}
                  className="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
                />
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300 flex items-center gap-2">
                  <Tag className="w-3.5 h-3.5 text-indigo-400" /> Etiquetas (separadas por comas)
                </label>
                <input
                  name="etiquetas"
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300 flex items-center gap-2">
                  <Hash className="w-3.5 h-3.5 text-indigo-400" /> Orden (Numérico)
                </label>
                <input
                  type="number"
                  name="orden"
            ...
            {ok && (
              <div className="flex items-center gap-2 p-3.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs rounded-xl">
                ...
                <span>¡Estudio añadido correctamente!</span>
              </div>
            )}
